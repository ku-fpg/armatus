...
public class TestCommandDispatcher {
	private static TestActivity mConsole;
	...
	//List of Keywords
	private static Keyword red = new Keyword("red", "toast", TestPrettyPrinter.RED);
	private static Keyword green = new Keyword("green", "toast", TestPrettyPrinter.GREEN);
	private static Keyword blue = new Keyword("blue", "toast", TestPrettyPrinter.BLUE);
	private static Map<String, Keyword> mKeywordMap = mapOfInstances(Keyword.class);

	public TestCommandDispatcher(TestActivity console) {
		mConsole = console;
	}
	...
	private static <T> Map<String, T> mapOfInstances(Class<T> instanceType) {
		...
		Field[] fields = TestCommandDispatcher.class.getDeclaredFields();
		for (Field f : fields) {
