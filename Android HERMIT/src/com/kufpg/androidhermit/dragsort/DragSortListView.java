...
package com.kufpg.androidhermit.dragsort;
...
import com.kufpg.androidhermit.multitouch.SimpleFloatViewManager;
...
public class DragSortListView extends ListView {
		...
		if (attrs != null) {
			...
			if (useDefault) {
				...
				boolean sortEnabled = a.getBoolean(
						R.styleable.DragSortListView_sort_enabled, false);
				...
				DragSortController controller = new DragSortController(this,
						dragHandleId, dragInitMode, removeMode);
	...
	private void measureItemAndGetHeights(int position, View item,
			ItemHeights heights) {
		...
		if (height > 0) {
			...
			if (isHeadFoot) {
				...
				if (height > 0) {
					...
					int hspec = MeasureSpec.makeMeasureSpec(0,
							MeasureSpec.UNSPECIFIED);
					int wspec = ViewGroup.getChildMeasureSpec(
