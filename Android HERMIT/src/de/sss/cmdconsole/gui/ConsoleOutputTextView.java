...
public class ConsoleOutputTextView extends ImageView implements
		ConsoleScrollView.ScrollViewListener {
	...
	private LinkedList<String> textBufferList; // /< buffer to hold all lines of
												// text
	private StringBuilder appendStringBuffer; // /< buffer used during appending
												// text
	private int lineLimitInView; // /< maximum number of lines which can be
									// viewed by user
	private int curViewYPos; // /< current view Y position (value callback by
								// scrollview)
	...
	public void appendText(String s) {
		...
		if (textBufferList.size() > 0) {
			...
			appendStringBuffer
					.append(textBufferList.get(textBufferList.size() - 1));
		...
		while (index < appendStringBuffer.length()) {
			...
			int tempCount = 0; // num of char that will accomodate in a single
								// line
			...
			if (lineBreakPos != -1) {
				// new line is found
				tempCount = textPaint.breakText(appendStringBuffer, index,
						lineBreakPos, true, maxWidth, null);
				if (tempCount < (lineBreakPos - index))
					lineBreakPos = -1; // linebreak not reached, leave for next
										// line
			} else {
				tempCount = textPaint.breakText(appendStringBuffer, index,
						appendStringBuffer.length(), true, maxWidth, null);
			}

			// Log.d(TAG, "tempCount = " + tempCount);

			textBufferList.add(appendStringBuffer.substring(index, index
					+ tempCount));
			index += (tempCount + ((lineBreakPos != -1) ? 1 : 0));
			searchStart = index;

			if (lineBreakPos != -1 && index == appendStringBuffer.length()) { // linebreak
																				// at
																				// last
																				// char
	...
	/*
	 * public int getLastLineYPos() { return DRAWYOFFSET + textBufferList.size()
	 * * LINEHEIGHT; }
	 * 
	 * public int getCurViewYPos() { return curViewYPos; }
	 */
	...
	public void setFontSize(int fontSize) {
		...
		for (ConstantData.ConsoleFontSize f : ConstantData.ConsoleFontSize
				.values()) {
			if (fontSize == f.v()) {
		...
		TEXTSIZE = (isValidFontSize) ? fontSize
				: ConstantData.ConsoleFontSize.SMALL.v();
		textPaint.setTextSize(TEXTSIZE);
		LINEHEIGHT = TEXTSIZE + 2;
		lineLimitInView = SCREENHEIGHT / LINEHEIGHT;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		float YOffset = DRAWYOFFSET; // don't draw at pixel 0 as it can't be
										// seen

		int startLine = curViewYPos / LINEHEIGHT;
		// Log.d(TAG, "startline = " + startLine + ", curViewYPos = " +
		// curViewYPos + ", text size = " + textBufferList.size());

		startLine = (startLine >= textBufferList.size()) ? (textBufferList
				.size() - 1) : startLine;
		...
		while (itr.hasNext()) {
			...
			++drawLineCount;
			if (drawLineCount > lineLimitInView) // no need to draw if out of
													// screen
				break;
		}
	...
	/**
	 * Callback by parent scrollView to notify that user has triggered a
	 * scrolling action
	 */
	@Override
	public void onViewScroll(int x, int y, int oldx, int oldy) {
		// Log.d(TAG, "x = " + x + ", y = " + y + ", oldx = " + oldx +
		// ", oldy = " + oldy);
		curViewYPos = y;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		int tempLineCount = textBufferList.size() < lineLimitInView ? lineLimitInView
				: textBufferList.size();
		int height = getDefaultSize(tempLineCount * LINEHEIGHT,
				heightMeasureSpec);
